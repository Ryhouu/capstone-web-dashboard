...
import { VerifiedSignatureLogDataSchema } from '../schema/PaymentSignatureDataSchema';
...
export default function VerifiedSignaturesTable({
    accountData,
    open
}: {
    ...
    const handleFetch = async () => {
        const res = await fetchWithToast(`/api/log/verified-signatures?account=${accountData.account}`, {
            method: 'GET',
        })
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = verifiedSignatureData.map((n) => n.id);
  ...
  // Avoid a layout jump when reaching the last page with empty rows.
    ...
    useEffect(() => {
        console.log("Updated verifiedSignatureData:", verifiedSignatureData);
        setEmptyRows(page > 0 ? Math.max(0, (1 + page) * rowsPerPage - verifiedSignatureData.length) : 0);
    }, [verifiedSignatureData]); 

    const preprocessedRows = React.useMemo(() => verifiedSignatureData.map(data => ({
        ...data,
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar numSelected={selected.length} main={"Verified Signatures"} />
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
          ...
          >
            <EnhancedTableHead
              ...
              rowCount={verifiedSignatureData.length}
            />
            <TableBody>
        ...
        <TablePagination
          ...
          component="div"
          count={verifiedSignatureData.length}
